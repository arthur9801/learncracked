(() => {
    ...
    function getRoot() {
        return (
            document.querySelector("#react-learncoach-app") ||
            document.body ||
    ...
    function findTimerButton() {
        ...
        const scored = candidates.map((btn) => {
            ...
            if (
                btn.querySelector(
                    'svg[class*="RoundProgress__SVG"], svg.RoundProgress__SVG-sc-yodzo6-0'
                )
            )
                score += 3;
    ...
    function removePauseModal(reason) {
        ...
        function tryReplacePauseButton() {
            ...
            try {
                ...
                const w = Math.max(
                    0,
                    Math.round(rect.width || parseFloat(cs.width) || 34)
                );
                const h = Math.max(
                    0,
                    Math.round(rect.height || parseFloat(cs.height) || 34)
                );
        ...
        const observer = new MutationObserver(() => {
            ...
            if (modal) {
                console.log(
                    logPrefix,
                    "Remove pause modal when it shows",
                    reason
                );
                modal.remove();
                observer.disconnect();
    ...
    function init() {
        try {
            ...
            window.addEventListener("pageshow", () =>
                console.log(logPrefix, "Page show", location.href)
            );
            window.addEventListener("load", () =>
                console.log(logPrefix, "Page load", location.href)
            );
